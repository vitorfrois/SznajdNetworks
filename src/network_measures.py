...
class NetworkMeasure:
    ...
    def __init__(self, network_file: str):
        ...
        self.nxgraph = self.igraph.to_networkx()
    ...
    def get_measure_dict(self) -> dict[str, float]:
        measure_dict = {
            ...
            # 'average_shortest_path_lenght': self.average_shortest_path_lenght({'directed': False}),
    ...
    def average_shortest_path_lenght(self, kwargs: dict | None = None) -> float:
        return self.igraph.average_shortest_path_lenght(**kwargs)
    ...
    def information_centrality(self, kwargs: dict | None = None) -> float:
        return np.mean(nx.information_centrality(self.nxgraph, kwargs).values())

    def approximate_current_flow_betweenness_centrality(self, kwargs: dict | None = None) -> float:
        return np.mean(nx.approximate_current_flow_betweenness_centrality(self.nxgraph, **kwargs).values())
